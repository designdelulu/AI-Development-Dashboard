...
Metric-definition version 3.0 adds private, structural-only `UsageObservation`, `WorkBlock`, `Attempt`, `OutcomeEvidence`, and `CapabilityEvidence` records. Evidence is independently labelled **Measured**, **Inferred**, **User-confirmed**, or **Unknown**. Prompt/response bodies, source code, terminal/test output, and tool arguments are never persisted. Command text may be inspected in memory only to recognize a bounded known validator; the stored record retains only its validator class and numeric exit status.

A Work Block is currently a **session proxy** with an explicit `session-proxy` boundary method. It is descriptive—not a Task—and is never silently promoted to a completed task. Attempts are created only around observed validator attempts. Passing validation is recorded as validation evidence, not proof that a task is correct or accepted. User-confirmed outcomes are reversible local metadata and survive rescans.

No universal efficiency/productivity score or model winner exists. The Efficiency workspace shows descriptive evidence with sample counts and suppresses comparative claims until equivalent task/outcome cohorts meet the documented thresholds. Provider-billed OpenRouter aggregates remain Exact, but are not assigned to a work block/project without deterministic correlation. Subscription usage/capacity is never converted to dollars. Capability presence is not invocation; only structured confirmed invocation supports an observational capability row. Efficiency is private and excluded from Share Stats.
