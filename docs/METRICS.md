...
Every token card inherits the selected range. Fresh + Output is labelled with that range (for example `Fresh + Output · Today`). Observed token activity includes cache reads/writes and is not billed usage. Contributor rows are a **selected-range observed-token distribution**: their printed percentage and bar width use the same denominator (`contributor observed activity / selected-range observed activity`). A non-zero contribution below 0.5% displays as `<1%`, never `0%`. This distribution is deliberately separate from the adaptive Fresh + Output intensity meter below. **Explain this number** shows range, timezone, exact vs estimated totals, category/agent/provider/model breakdown, source event counts, and contributing sessions without prompt bodies.

### Adaptive Token Activity intensity

The Live Feed intensity meter is a **visualization aid**, not a token metric or quota meter. It uses **Fresh + Output** so cache/context processing cannot make ordinary new work look artificially maxed out. The normalized history currently retains comparable **one local calendar-day** usage buckets, so the meter compares the current local day with completed local calendar days; it does not pretend a selected 7-day/month report is a same-size live bucket. It never sets contributor-share bar widths.
...
- **Working** — a real local event was observed within the previous 12 seconds, or a currently valid structured in-progress hold says a Claude tool has not completed. This is observed local busy activity, not remote model inference.
- **Needs You** — only a positive, structured attention marker can produce this state. It is never inferred from silence, process idleness, or a completed turn. Codex `event_msg` records with `payload.type: task_complete` are completion evidence and do **not** qualify; only an adapter allowlisted explicit approval, permission, input, or user-action request can create the state. An unresolved marker is held only while current (with a bounded safety expiry) and clears on structured resolution, subsequent work, completion, or process exit. This describes a present local request for action, not historical activity or remote model state.
- **Recently Active** — the last observed local event is more than 12 seconds and at most 30 seconds old, with no qualifying attention marker. This is a short transition after an agent stops producing local activity; it is not a multi-minute history label.
- **Idle** — the declared local runtime executable is currently present, but no validated AI work is occurring. The UI separately says when the last AI activity was observed.
- **Closed** — a reliable local executable-only process snapshot did not find the declared runtime. The UI separately says when the last AI activity was observed; it does not imply the runtime has been closed for that duration.
- **Presence Unknown** — the adapter has a live-work source but cannot safely determine whether its runtime is open.
- **Unknown** — the normalized index does not establish a supported local source for that agent.

Claude's structured `assistant/end_turn` and Cursor's `turn_ended/success` records show that a turn ended, but neither safely proves a user action is required, so they remain Recently Active/Idle. Cursor's editor/WAL activity is never used for Needs You. The dashboard reads only bounded structural JSONL fields for Codex attention—event type and payload type—not prompt, message, transcript, or tool-argument bodies; `task_complete` is explicitly treated as resolution/completion, while only allowlisted approval, permission, input, or user-action request types can create an unresolved marker.

Browser-local timing begins only when this state tracker first runs. A versioned fixed-size record in local storage accumulates `observedWorkingMs`, `waitingForUserMs`, `recentlyActiveMs`, `observedIdleMs`, `unknownMs`, and `unobservedMs`, plus at most 96 recent state transitions. It never backfills historical durations. Tick gaps over five seconds—such as a suspended or closed tab—go to `unobservedMs` rather than being credited to a state. These measurements describe what the open dashboard observed and form a conservative foundation for future project/cycle attribution; they are not exact provider response or compute times. Closed and Presence Unknown are visual/runtime-presence states; they are not credited as observed work or idle duration.

### Live signal sources

The backend checks the known live files every 1.5 seconds and the browser requests the compact live snapshot every two seconds. Process enumeration is asynchronous and refreshed at most every five seconds; a short stale-good window prevents one slow/failed `ps` call from turning every runtime Unknown. It reads `ps comm` executable paths—not command arguments, terminal content, project paths, prompts, or tool input—and applies only adapter-declared process hints. Process presence can choose Idle versus Closed; it never creates a pulse, Working state, token activity, attention marker, or global live signal. Claude pulses come only from **size growth** of `.claude/projects/**/*.jsonl` session transcripts (including subagent JSONL). mtime-only touches, missing files, directory watches, and dashboard/statusline/config files (`~/.claude/ai-dashboard/**`, `usage_state.json`, settings backups) never generate Claude pulses. When a newly appended Claude JSONL record structurally starts a `tool_use` and its matching `tool_result` has not yet appeared, an in-memory, bounded five-minute **in-progress hold** keeps Claude in Working state. It stores only a transient tool ID and timestamp—never command input, tool arguments, output, prompts, or text—and expires rather than pinning an abandoned terminal as Working. Codex uses modified `.codex/sessions/**/*.jsonl` files.

Cursor activity requires **growing local agent evidence**, not application startup or editor storage:

- size growth of `~/.cursor/projects/**/agent-tools/*`, or
- growth of a JSONL `agent-transcripts` record structurally typed `user` or `assistant`.

Cursor `turn_ended`-only startup markers, generic source-directory watches, `state.vscdb`, `state.vscdb-wal`, SQLite WAL mtime changes, project-window opening, and extension housekeeping do not create a live pulse. When appended transcript rows structurally identify a user/assistant turn start, an in-memory bounded five-minute turn hold keeps the lane Working through sparse planning/tool periods until a structural turn-complete/status-success row arrives; it retains no transcript body. The live path reads only the bounded top-level structural row type for agent transcripts. Token analytics may still read `state.vscdb` **read-only** (YELLOW undocumented local storage) for `cursorDiskKV` `bubbleId:` / `composerData:` metadata only: token counts, context-meter totals, timestamps, model ids, and text lengths. It does not read ItemTable auth keys, cookies, JWTs, or prompt bodies. `canvases/`, `mcps/`, and `node_modules` are ignored. A valid file update has a deterministic base weight plus a capped log-scale byte-growth contribution. It is evidence of local session output/activity, not remote compute.
