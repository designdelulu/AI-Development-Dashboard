...
Current implementation also includes a feature-probed Cline local adapter. Cline is an agent, not an exclusive host for OpenRouter-routed inference: when installed inside Cursor the normalized path is `Cline → Cursor → OpenRouter → provider → model`, while Claude Code, Codex, or another supported host may use the same gateway independently. The adapter feature-probes the actual Cursor/VS Code extension roots and reads bounded structural session JSON/JSONL metadata and exact numeric usage fields where exposed. For the validated Cursor-hosted Cline 4.1.14 runtime it also uses a separate read-only helper that selects only the session lifecycle columns from `~/.cline/data/db/sessions.db`; message/prompt/metadata columns, `*.messages.json` bodies, provider settings, and inference credentials remain outside its read boundary. Cline has no capacity source of its own.
...
| Cline | Yes: CLI/root/Cursor or VS Code extension | Partial; feature-probed session JSON/JSONL plus reviewed 4.1.14 lifecycle DB columns | Structured session lifecycle when exposed; current Cursor-hosted 4.1.14 uses a narrow read-only session DB heartbeat | Partial; exact numeric fields only when the session artifact exposes matching semantics | Unavailable locally; OpenRouter billed aggregate remains separate | No Cline source | Exact observed IDs; configured route is not usage | Recorded workspace/cwd only when below known project | Partial; schema/version dependent; host is Cursor when extension evidence is unambiguous |
