...
Current implementation also includes a feature-probed Cline local adapter. Cline is an agent, not an exclusive host for OpenRouter-routed inference: when installed inside Cursor the normalized path is `Cline → Cursor → OpenRouter → provider → model`, while Claude Code, Codex, or another supported host may use the same gateway independently. The adapter feature-probes the actual Cursor/VS Code extension roots and reads only bounded structural session JSON/JSONL metadata and exact numeric usage fields where exposed; `*.messages.json` bodies, SQLite session indexes, provider settings, and inference credentials remain outside its read boundary. Cline has no capacity source of its own.
