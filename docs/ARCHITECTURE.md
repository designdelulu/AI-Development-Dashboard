...
Runtime presentation is derived from adapter manifests and normalized sessions. A manifest may declare a runtime/source key and validated live capability; the UI renders only installed or historically observed local runtimes with that capability. Connected gateways such as OpenRouter are not runtimes and never become a live lane merely because their aggregate history changes. Capacity cards are rendered from returned account/capacity sources, not model names.
