...
Live work is driven only by an unexpired row in Hermes's durable `session_turn_leases` table, resolved to an allowlisted active session. `runtime/active_sessions.json` is read only for matching session ID and observed surface; it identifies a CLI/TUI/Desktop/Gateway host but does not itself mean a turn is running. This distinguishes an idle open surface/session slot from Working and keeps a quiet OpenRouter model wait or tool run Working until Hermes releases/expires the actual turn lease. When an observed lease disappears, the dashboard emits a structural completion signal that decays through Recently Active; no waveform is fabricated. Explicit Hermes attention is not surfaced because the validated structural source does not provide a privacy-safe current approval/input marker.
