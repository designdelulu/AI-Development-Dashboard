...
Cline is a local agent adapter, not a capacity source. It detects a Cline CLI, the actual Cursor or VS Code extension installation, and the documented `~/.cline/data/sessions/` area while Cline is closed. In the validated owner installation, the extension is `saoudrizwan.claude-dev` 4.1.14 under `~/.cursor/extensions/`, with Cursor global storage under `~/Library/Application Support/Cursor/User/globalStorage/saoudrizwan.claude-dev`; the normalized host is `Cursor` and vanilla VS Code is not required. The adapter uses the extension metadata only for installation/host evidence and reads session snapshots from `~/.cline/data/sessions/`, not global-storage bodies. Current safe parsing is deliberately narrow: bounded structural JSON/JSONL session snapshots and numeric usage fields are read when present; `*.messages.json` transcript bodies are excluded before opening, and a detected `sessions.db` index is recorded as an available source but is not opened by this adapter. Prompt/response bodies, transcript content, tool arguments, provider settings, secret stores, and inference keys are never read.

When a session record identifies a route, the normalized identity preserves `agent: Cline`, `host: Cursor` (or another explicitly evidenced host), `gateway: OpenRouter`, the underlying provider, and the raw/normalized model ID separately. Cline model configuration is **Configured** evidence only; a session record is required before the model becomes **Observed** usage. Workspace/cwd attribution is Confirmed only when it resolves below a known project root. A structured active/running/planning/tool state can hold a Cline lane in Working; a fresh active snapshot may bootstrap that state after a restart, while an old active snapshot remains watch-only until a new structural change. Once started, the lane uses a bounded 15-minute quiet lease measured from the last validated snapshot progress—not from original turn start—so sparse remote-model waits and long tools remain Working without fabricating pulses. Completion/cancellation clears the lease immediately; an unchanged snapshot cannot re-arm an expired lease. Cursor process presence can distinguish the installed Cline extension's host as present/closed, but it never creates Cline activity; only Cline session evidence does.
