...
## Hermes Agent local adapter (feature-probed)

Validated against Hermes Agent 0.20.6. Hermes discovery separately records installation, retained history, configured route, runtime presence, and current work. It supports the default Hermes home and future profile homes without exposing raw profile labels. Configuration is parsed only for the non-secret `model.provider` and `model.default` fields; it does not open `.env` or read a credential value. A configured model is never promoted to observed history.

History uses a read-only SQLite helper against the canonical state database. It queries only allowlisted `sessions` and `session_model_usage` columns: structural IDs/source, observed model, lifecycle timestamps, numeric token categories, tool/API counts, safe project-attribution roots, and billing gateway. It never queries `messages`, FTS tables, system prompts, session titles, model-config JSON, user identifiers, routing payloads, or any message/tool/memory content. Missing or future schemas degrade to detected/telemetry unavailable. Exact token categories are retained only where Hermes exposes matching numeric fields; local Hermes cost stays unavailable, and remote OpenRouter analytics is never correlated by timestamp.

Live work is driven only by an unexpired row in Hermes's durable `session_turn_leases` table, resolved to an allowlisted active session. Hermes 0.20.6 documents this lease as covering the full load/run/flush region and refreshes it through long model, tool, and compression work. `runtime/active_sessions.json` is read only for matching session ID and observed surface; it identifies a CLI/TUI/Desktop/Gateway host but does not itself mean a turn is running. This distinguishes an idle open surface/session slot from Working and keeps a quiet OpenRouter model wait or tool run Working until Hermes releases/expires the actual turn lease. A failed narrow read is explicitly distinct from a successful empty lease query: the dashboard retains a previously validated turn only until Hermes's own persisted expiry, and creates completion only after a successful structural observation. When a lease genuinely disappears, the dashboard emits a structural completion signal that decays through Recently Active; no waveform is fabricated. Explicit Hermes attention is not surfaced because the validated structural source does not provide a privacy-safe current approval/input marker.

Cursor live work is separately based on current built-in AI transcript lifecycle rows under its agent-transcript store: structural `message.role` start evidence and terminal `turn_ended` status. The watcher parses a bounded fresh transcript tail for a new file or dashboard-restart baseline, so an active remote wait restores as Working without treating editor typing, file saves, generic Cursor storage churn, or Cline activity as Cursor work. Old transcript tails are watchable but cannot bootstrap a new Working state.

`/api/live-state` exposes a bounded in-memory decision trace for Hermes and Cursor. Each entry contains only timestamp, adapter, hashed session/artifact ID, host/surface, raw lifecycle label, last structural timestamp, normalized state, and reason code. This helps distinguish `runtime-present`, `turn-running`, `completion`, `no-active-turn`, and unavailable/unsupported probes without retaining conversation content.
