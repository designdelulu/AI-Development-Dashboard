...
Cline is a separate local inference host. The dashboard does not read Cline's OpenRouter/provider inference key, VS Code secret storage, provider settings, or message bodies. A Cline session may identify `Cline → OpenRouter → provider → model` for local metadata, but account-level OpenRouter analytics is never assigned to Cline (or any other host) from timestamps alone. Any future correlation must use an explicit shared request identifier and a reviewed allowlist.
