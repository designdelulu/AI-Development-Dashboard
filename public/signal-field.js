...
export function signalBarSample(events,agent,sampleAt,index,{reducedMotion=false}={}){
  ...
  const baseline=reducedMotion?.006:.007+.013*(.5+.5*Math.sin(sampleAt/620+agentPhase+index*.19));
  ...
  return {realEnergy,visualEnergy,baselineOnly:realEnergy===0,amplitude:Math.min(1,baseline+visualEnergy*(.28+.72*carrier)*gate),opacity:realEnergy===0?.1:.24+.68*visualEnergy,density};
}
