...
function realpathOrNull(value, realpath = fs.realpathSync) {
  try { return realpath(value); } catch { return value ? path.resolve(value) : null; }
}

function packageRoot(value, realpath = fs.realpathSync) {
  const resolved = realpathOrNull(value, realpath);
  if (!resolved) return null;
  let current = path.dirname(resolved);
  for (let i = 0; i < 5; i++) {
    try { if (fs.existsSync(path.join(current, 'package.json'))) return current; } catch {}
...
/** Invocation may use the linked bin path or src/cli.js from the same checkout. */
export function scriptsEquivalent(left, right, { realpath = fs.realpathSync } = {}) {
  if (!left || !right) return false;
  const a = realpathOrNull(left, realpath);
  const b = realpathOrNull(right, realpath);
  if (a && b && a === b) return true;
  const aRoot = packageRoot(left, realpath);
  const bRoot = packageRoot(right, realpath);
...
export function validRuntime(record, { script = null } = {}) {
  return Boolean(record && (!record.service || record.service === 'ai-development-dashboard') && Number.isInteger(record.pid) && record.pid > 0 && processAlive(record.pid) && (!script || scriptsEquivalent(record.script, script) || record.script === script));
}
...
export function ownsProcess(record, { script = null, command = processCommand } = {}) {
  ...
  if (script && !scriptsEquivalent(record.script, script) && record.script !== script) return false;
  ...
  if (expected && (normalized.includes(expected) || normalized.includes(path.basename(expected)))) return true;
  return Boolean(record.service === 'ai-development-dashboard' && /(?:^|\/)ai-dashboard(?:\.js)?(?:\s|$)/i.test(normalized) && /(?:^|\s)serve(?:\s|$)/i.test(normalized));
}
