...
export function discoverClosedTools({ homedir = os.homedir(), env = process.env, platform = process.platform } = {}) {
  ...
  const result = {
    ...
    Antigravity: sourceState({ installed: Boolean(app('Antigravity') && exists(app('Antigravity'))) || exists(path.join(homedir, '.gemini', 'antigravity')), appPath: app('Antigravity'), binary: cli('agy', ['/opt/homebrew/bin', '/usr/local/bin']), historyPaths: [path.join(homedir, '.gemini', 'antigravity')], historySupported: false, needsSession: true }),
