...
export function liveStatesFromEvents(events = [], agents = [], now = Date.now(), attentionSignals = {}, inProgressSignals = {}, presenceStates = {}, telemetryUnavailableSignals = {}, completionSignals = {}) {
  const names = [...new Set([...(agents || []), ...events.map((event) => event.agent).filter(Boolean)])];
  return Object.fromEntries(names.map((agent) => [agent, classifyAgentState(events, agent, now, { sourceKnown: true, attention: attentionSignals[agent] || null, inProgress: inProgressSignals[agent] || null, completion: completionSignals[agent] || null, telemetryUnavailable: telemetryUnavailableSignals[agent] || null, presence: presenceStates[agent] || null })]));
}
...
export function buildOperator(index, events = [], capacity = null, { now = Date.now(), availableAgents = [], attentionSignals = {}, inProgressSignals = {}, completionSignals = {}, telemetryUnavailableSignals = {}, presenceStates = {} } = {}) {
  ...
  const liveStates = liveStatesFromEvents(events, liveAgents, now, attentionSignals, inProgressSignals, presenceStates, telemetryUnavailableSignals, completionSignals);
