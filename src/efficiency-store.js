...
import { OUTCOME_STATES, EFFICIENCY_EVIDENCE, normalizeProspectiveComparison } from './efficiency.js';
...
const cycle = (item = {}) => {
  ...
  return {
    ...
    taskCategory: bounded(item.taskCategory, 64),
    ...
    capabilityConfigurationKnown: item.capabilityConfigurationKnown === true,
...
export function createCycle(metadata, workBlockIds = [], details = {}) { const ids = [...new Set(workBlockIds.filter(Boolean))]; const privateLabel = bounded(details.privateLabel, 160); if (!ids.length) throw new Error('Select at least one work block to create a cycle.'); if (ids.length < 2 && !privateLabel && !bounded(details.taskKey, 128)) throw new Error('Select at least two work blocks, or provide a private objective for a comparison cycle.'); const next = clean(metadata), taskKey = bounded(details.taskKey, 128) || (privateLabel ? `task:${hash(privateLabel)}` : null), created = cycle({ id: `cycle:${hash(ids.slice().sort().join(':'))}`, workBlockIds: ids, boundaryMethod: 'user-confirmed-grouping', boundaryConfidence: 'user-confirmed', createdAt: new Date().toISOString(), ...details, privateLabel, taskKey }); next.cycles = [...next.cycles.filter((item) => item.id !== created.id), created]; return { metadata: next, cycle: created }; }
...
export function applyEfficiencyMetadata(foundation = {}, metadata = {}) { const next = clean(metadata); const outcomes = [...(foundation.outcomes || [])]; for (const [workBlockId, value] of Object.entries(next.outcomes)) outcomes.push({ id: `outcome:user:${hash(workBlockId)}`, workBlockId, state: value.state, evidenceClass: 'user-confirmed', checks: [], recordedAt: value.recordedAt });
  ...
  const normalized = normalizeProspectiveComparison({ ...foundation, outcomes }, { instrumentationStartedAt: next.comparison.instrumentationStartedAt, userCycles: cycles });
  return { ...normalized, userCycles: cycles, controlledExperiments: next.experiments, comparison: next.comparison };
}
