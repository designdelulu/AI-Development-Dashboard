...
Hermes Agent's inference credential, `.env`, auth data, memory files, SOUL/USER files, logs, message bodies, tool calls/results, and search/FTS tables are never read. The Hermes adapter feature-probes its local home and reads only a reviewed SQLite allowlist: session ID/source/model/lifecycle timestamps, numeric token counters, safe structural project roots/cwds for deterministic local attribution, observed billing gateway, and current durable turn-lease timestamps. It also reads only session ID/surface from the bounded active-session registry; registry metadata is excluded. Database/home/profile paths are not retained in dashboard session records, and profile display is anonymized. A current durable turn lease can represent live work; an open Hermes surface, active session routing entry, or configured model cannot. Hermes's OpenRouter credential remains wholly inside Hermes, separate from the dashboard's optional OpenRouter Management key. Local Hermes rows are never joined to account analytics by time.

The live-state decision trace for Hermes and Cursor is bounded in memory and may enter an opted-in bug-report diagnostic bundle. It contains only timestamps, adapter names, hashed structural session/artifact IDs, host/surface, lifecycle labels, normalized states, and reason codes. It excludes paths, prompt/response text, source code, terminal contents, tool arguments/results, credentials, and all raw session IDs.
...
- Hermes prompts/messages, tool data, memories, SOUL/USER files, `.env`, auth data, logs, full home/profile paths, or inference credentials
