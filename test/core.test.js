...
test('structured Codex completion is the only current attention marker and clears on later work',()=>{const at=3_000_000,marker=structuredAttentionFromRows('Codex',[{type:'event_msg',payload:{type:'task_complete'}}],at);assert.equal(marker?.kind,'codex-task-complete');assert.equal(structuredAttentionFromRows('Claude',[{type:'assistant',message:{stop_reason:'end_turn'}}],at),null);assert.equal(structuredAttentionFromRows('Cursor',[{type:'turn_ended',status:'success'}],at),null);const completed={agent:'Codex',timestamp:new Date(at).toISOString()};assert.equal(classifyAgentState([completed],'Codex',at,{attention:marker}).state,'Needs You');const resumed={agent:'Codex',timestamp:new Date(at+21_000).toISOString()};assert.equal(classifyAgentState([completed,resumed],'Codex',at+25_000,{attention:marker}).state,'Working');assert.equal(structuredAttentionFromRows('Codex',[{type:'event_msg',payload:{type:'task_complete'}},{type:'event_msg',payload:{type:'task_started'}}],at),null);assert.equal(structuredAttentionFromRows('Codex',[{type:'event_msg',payload:{type:'task_complete'}},{type:'response_item',payload:{type:'message'}}],at),null);});
test('attention marker has stable hysteresis without polling flicker',()=>{const at=3_000_000,event={agent:'Codex',timestamp:new Date(at).toISOString()},attention={agent:'Codex',at:new Date(at).toISOString(),kind:'codex-task-complete',reason:'Structured completion'};const a=classifyAgentState([event],'Codex',at+20_000,{attention}),b=classifyAgentState([event],'Codex',at+200_000,{attention});assert.equal(a.state,'Needs You');assert.equal(b.state,'Needs You');assert.equal(classifyAgentState([event],'Codex',at+901_000,{attention}).state,'Idle');});
