...
import { boundedSignalEvents, eventSignalEnvelope, mergeMonitorEvents, signalBarSample, signalEnergy } from '../public/signal-field.js';
...
import { createPresenceSampler, presenceSamplerKey, processNameProbeCommand, processNameSnapshot, processSnapshot, processSnapshotCommand, runtimePresenceStates, PRESENCE_STALE_GOOD_MS } from '../src/runtime-presence.js';
...
import { writeChunkedText } from '../src/response-stream.js';
import { createLiveRuntimeStore } from '../public/live-runtime-store.js';
import { claudeLiveDecision, cursorLiveDecision, isClaudeLivePath, isCursorLivePath, isCursorUnsupportedSurfacePath, isDashboardGeneratedClaudePath } from '../src/live-files.js';
...
test('equivalent runtime refreshes retain one presence-sampler identity while a changed matcher does not',()=>{const cursor={id:'cursor',agent:'Cursor',liveCapable:true,presence:{processPathSuffixes:['cursor.app/contents/macos/cursor']}},same={...cursor,presence:{...cursor.presence}},changed={...cursor,presence:{processPathSuffixes:['cursor.app/contents/macos/cursor-beta']}};assert.equal(presenceSamplerKey([cursor]),presenceSamplerKey([same]));assert.notEqual(presenceSamplerKey([cursor]),presenceSamplerKey([changed]));});
test('macOS main-executable probe separates present, absent, and unavailable without exposing PIDs',()=>{assert.deepEqual(processNameProbeCommand('Cursor','darwin'),{command:'/usr/bin/pgrep',args:['-ix','Cursor']});assert.deepEqual(processNameSnapshot('cursor',{now:()=>0}),{reliable:true,commands:['cursor'],checkedAt:new Date(0).toISOString(),reason:null});assert.deepEqual(processNameSnapshot('cursor',{error:{code:1},now:()=>0}),{reliable:true,commands:[],checkedAt:new Date(0).toISOString(),reason:null});assert.equal(processNameSnapshot('cursor',{error:{killed:true},now:()=>0}).reliable,false);});
test('a validated current turn outranks temporary unknown presence while confirmed closure clears it',()=>{const now=7_000_000,turn={active:true,since:new Date(now-1_000).toISOString(),confidence:'Structured'};assert.equal(classifyAgentState([],'Cursor',now,{presence:{state:'unknown'},inProgress:turn}).state,'Working');assert.equal(classifyAgentState([],'Cursor',now,{presence:{state:'closed',checkedAt:new Date(now).toISOString()},inProgress:turn}).state,'Closed');});
test('large cached responses yield between bounded writes without changing their payload',async()=>{const writes=[],scheduled=[];let ended=false;const response={destroyed:false,write:(chunk)=>{writes.push(chunk);return true;},end:()=>{ended=true;}};writeChunkedText(response,'abcdefghij',{chunkBytes:3,schedule:(fn)=>scheduled.push(fn)});while(scheduled.length)scheduled.shift()();assert.deepEqual(writes,['abc','def','ghi','j']);assert.equal(ended,true);});
test('live runtime overlay retains runtime-only lanes across hydration, failures, 304s, and stale responses',()=>{const base={liveRuntimes:[{id:'claude',agent:'Claude',liveCapable:true}]},cursor={id:'cursor',agent:'Cursor',liveCapable:true},cline={id:'cline',agent:'Cline',host:'Cursor',liveCapable:true},hermes={id:'hermes',agent:'Hermes Agent',liveCapable:true},store=createLiveRuntimeStore({now:()=>0});store.applyBase(base,{source:'initial-data'});const first=store.beginLiveRequest();assert.equal(store.applyLive({liveRuntimes:[cursor,cline,hermes]},{request:first}),true);assert.deepEqual(store.current().liveRuntimes.map(x=>x.agent),['Cursor','Cline','Hermes Agent']);store.applyBase(base,{source:'delayed-data'});assert.deepEqual(store.current().liveRuntimes.map(x=>x.agent),['Cursor','Cline','Hermes Agent']);store.retainLive({mutation:'live-304-retained'});store.retainLive({mutation:'live-fetch-failed-retained'});assert.equal(store.current().liveRuntimes.filter(x=>x.agent==='Cursor').length,1);const older=store.beginLiveRequest(),newer=store.beginLiveRequest();assert.equal(store.applyLive({liveRuntimes:[cursor]},{request:newer}),true);assert.equal(store.applyLive({liveRuntimes:[{id:'claude',agent:'Claude',liveCapable:true}]},{request:older}),false);assert.deepEqual(store.current().liveRuntimes.map(x=>x.agent),['Cursor']);assert.match(store.diagnostics().trace.map(x=>x.mutation).join(','),/base-updated-live-retained/);});
...
test('live hook bursts retain same-second events while historical overlap remains deduplicated',()=>{const duplicate={agent:'Cursor',kind:'cursor-hook-afterAgentThought',timestamp:'2026-08-30T14:58:37.000Z'},base={agent:'Cursor',kind:'session-file-update',timestamp:'2026-08-30T14:58:37.000Z'};const merged=mergeMonitorEvents([base],[duplicate,duplicate,base]);assert.equal(merged.length,3);assert.equal(merged.filter(event=>event.kind==='cursor-hook-afterAgentThought').length,2);assert.equal(merged.filter(event=>event.kind==='session-file-update').length,1);});
...
test('cursor live allowlist rejects storage housekeeping and requires growing agent evidence',()=>{
  ...
  assert.equal(isCursorUnsupportedSurfacePath('/Users/x/.cursor/projects/opaque/terminals/opaque.txt'),true);
  assert.equal(isCursorLivePath('/Users/x/.cursor/projects/opaque/terminals/opaque.txt'),false);
...
test('the browser entry module parses before it is served',()=>{
  for (const file of ['app.js','live-ui.js','live-runtime-store.js','resume-ui.js','agent-state.js','signal-field.js']) {
